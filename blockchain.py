...
# Adding a new transaction to the last block
...
def connect_node():
    ...
    if nodes is None:
        return "No available node", 400
    for node in nodes:
        blockchain.add_node(node)
    response = {'message': f'All the nodes are now connected! The size of the blockchain is {list(blockchain.nodes)}'}
    return response
